# tf-atom-vpc-endpoint-aws

[![Terraform Format](https://img.shields.io/badge/terraform-fmt-blue?logo=terraform)](https://github.com/PlatformStackPulse/tf-atom-vpc-endpoint-aws/actions)
[![Terraform Validate](https://img.shields.io/badge/terraform-validate-blue?logo=terraform)](https://github.com/PlatformStackPulse/tf-atom-vpc-endpoint-aws/actions)
[![TFLint](https://img.shields.io/badge/tflint-passing-brightgreen?logo=terraform)](https://github.com/PlatformStackPulse/tf-atom-vpc-endpoint-aws/actions)
[![Terraform Test](https://img.shields.io/badge/tests-2%20passed-brightgreen?logo=terraform)](https://github.com/PlatformStackPulse/tf-atom-vpc-endpoint-aws/actions)
[![Security Scan](https://img.shields.io/badge/trivy-passing-brightgreen?logo=aqua)](https://github.com/PlatformStackPulse/tf-atom-vpc-endpoint-aws/actions)
...
[![Documentation](https://img.shields.io/badge/docs-terraform--docs-blue?logo=readthedocs)](https://github.com/PlatformStackPulse/tf-atom-vpc-endpoint-aws/actions)
...
A tf-label-conventioned atom module that provisions a single **AWS VPC Endpoint** (`aws_vpc_endpoint`) — Interface, Gateway, or Gateway Load Balancer — with consistent naming/tagging and a conditional `enabled` toggle.

## Features

- **All three endpoint types** — `Interface`, `Gateway`, and `GatewayLoadBalancer`, validated on input
- **Type-aware options** — Interface options (`private_dns_enabled`, `security_group_ids`, `subnet_ids`) and Gateway options (`route_table_ids`) applied only when relevant
- **Endpoint IAM policy** — Optional `policy` (JSON) to restrict access through the endpoint
- **Auto-accept control** — `auto_accept` for endpoints requiring acceptance
- **tf-label naming & tagging** — namespace/stage/name → deterministic `id` and merged tags via the `this` context module
- **Conditional creation** — `enabled = false` provisions nothing; outputs degrade to empty values
- **Rich outputs** — `id`, `arn`, `dns_entry`, `network_interface_ids`, `prefix_list_id`, and `enabled`
- **Native Terraform testing** — `terraform test` unit tests with a mock provider (no AWS calls)
- **Quality gates** — TFLint, Trivy IaC scanning, terraform-docs freshness, conventional commits, pre-commit hooks
...
### Interface endpoint (private connectivity to an AWS service)

```hcl
module "ssm_endpoint" {
  source = "git::https://github.com/PlatformStackPulse/tf-atom-vpc-endpoint-aws.git?ref=v1.0.0"
...
### Gateway endpoint (S3 / DynamoDB via route tables)
...
`vpc_id` and `service_name` are the only required inputs; all other module-specific
inputs default to values suited to an Interface endpoint.
...
|------|-------------|------|---------|:--------:|
| <a name="input_service_name"></a> [service\_name](#input\_service\_name) | The AWS service name (e.g. com.amazonaws.us-east-1.s3). | `string` | n/a | yes |
| <a name="input_vpc_id"></a> [vpc\_id](#input\_vpc\_id) | The VPC ID in which to create the endpoint. | `string` | n/a | yes |
| <a name="input_attributes"></a> [attributes](#input\_attributes) | ID element. Additional attributes (e.g. `workers` or `cluster`) to add to `id`,<br/>in the order they appear in the list. New attributes are appended to the<br/>end of the list. The elements of the list are joined by the `delimiter`<br/>and treated as a single ID element. | `list(string)` | `[]` | no |
| <a name="input_auto_accept"></a> [auto\_accept](#input\_auto\_accept) | Whether to auto-accept the VPC endpoint connection. | `bool` | `null` | no |
...
| <a name="input_policy"></a> [policy](#input\_policy) | IAM policy document for the endpoint. | `string` | `null` | no |
| <a name="input_private_dns_enabled"></a> [private\_dns\_enabled](#input\_private\_dns\_enabled) | Whether to enable private DNS for Interface endpoints. | `bool` | `true` | no |
| <a name="input_regex_replace_chars"></a> [regex\_replace\_chars](#input\_regex\_replace\_chars) | Terraform regular expression (regex) string.<br/>Characters matching the regex will be removed from the ID elements.<br/>If not set, `"/[^a-zA-Z0-9-]/"` is used to remove all characters other than hyphens, letters and digits. | `string` | `null` | no |
| <a name="input_route_table_ids"></a> [route\_table\_ids](#input\_route\_table\_ids) | Route table IDs for Gateway endpoints. | `list(string)` | `[]` | no |
| <a name="input_security_group_ids"></a> [security\_group\_ids](#input\_security\_group\_ids) | Security group IDs for Interface endpoints. | `list(string)` | `[]` | no |
| <a name="input_stage"></a> [stage](#input\_stage) | ID element. Usually used to indicate role, e.g. 'prod', 'staging', 'source', 'build', 'test', 'deploy', 'release'. | `string` | `null` | no |
| <a name="input_subnet_ids"></a> [subnet\_ids](#input\_subnet\_ids) | Subnet IDs for Interface endpoints. | `list(string)` | `[]` | no |
...
| <a name="input_vpc_endpoint_type"></a> [vpc\_endpoint\_type](#input\_vpc\_endpoint\_type) | Type of VPC endpoint. Valid values: Gateway, Interface, GatewayLoadBalancer. | `string` | `"Interface"` | no |

### Outputs

| Name | Description |
|------|-------------|
| <a name="output_arn"></a> [arn](#output\_arn) | The ARN of the VPC endpoint. |
| <a name="output_dns_entry"></a> [dns\_entry](#output\_dns\_entry) | DNS entries for the VPC endpoint. |
| <a name="output_enabled"></a> [enabled](#output\_enabled) | Whether the module is enabled. |
| <a name="output_id"></a> [id](#output\_id) | The ID of the VPC endpoint. |
| <a name="output_network_interface_ids"></a> [network\_interface\_ids](#output\_network\_interface\_ids) | Network interface IDs for Interface endpoints. |
| <a name="output_prefix_list_id"></a> [prefix\_list\_id](#output\_prefix\_list\_id) | The prefix list ID for Gateway endpoints. |
<!-- END_TF_DOCS -->

## Tests

Unit tests live in [`tests/unit/main_test.tftest.hcl`](tests/unit/main_test.tftest.hcl) and run
against a `mock_provider "aws"` — no AWS credentials or network calls are required. They assert on
plan-known values only (tf-label `id`, the `enabled` output, resource count, and input pass-throughs),
never on computed `arn`/`id` which are unknown under a mock provider.

Covered cases:

- `creates_when_enabled` — exactly one `aws_vpc_endpoint` is planned, `enabled` output is `true`, and `service_name` is passed through.
- `disabled_creates_nothing` — with `enabled = false`, no endpoint is created and `id` degrades to `""`.
...
Integration tests (real AWS) live under `tests/integration/` and run via `make test-integration`.
